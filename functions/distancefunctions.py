...
def mindist_pairwise_distances(alpha, beta):
    ## here alpha and beta are two chain objects
    distances = {}
    for resa in alpha:
        if (resa.id[0] == " ") & (seq1(resa.resname) != 'X'): # this avoids hetero-residues which don't have CA
            try:
                resaID = "".join([str(resa.id[1]), resa.id[2]]).strip()
                for resb in beta:
                    try:
                        if (resb.id[0] == " ") & (seq1(resb.resname) != 'X'): # this avoids hetero-residues
                            resbID = "".join([str(resb.id[1]), resb.id[2]]).strip()
                            dists = []
                            for atom1 in resa:
                                if atom1.id != 'H':
