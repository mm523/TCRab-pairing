...
def test_BioPDBdistancefunction_mindist():
    p = PDB.PDBParser()
    pymol.cmd.set('fetch_path', structures_dir1, quiet=0)
    for pdb in pdblist:
        print(pdb)
        ...
        for pair in chain_pairs:
            ...
            print(pair)
            achain, bchain = pair[0], pair[1]
            d1 = df.mindist_pairwise_distances(struc[0][achain],struc[0][bchain])
            d1pymol = df.pairwise_dist_pymol(pdb + " and chain " + achain, pdb + " and chain " + bchain, "3000")
            d1imgt = df.mindist_pairwise_distances(struc_imgt[0][achain],struc[0][bchain])

            d1imgt_ca = df.CA_pairwise_distances(struc_imgt[0][achain],struc[0][bchain])

            assert len(d1imgt) == len(d1imgt_ca)
            ...
            print(d)
            print(d.loc[d.raw > d.pymol])

            assert all(d.raw <= d.pymol + 0.15)
            assert all(d.imgt <= d.pymol + 0.15)     

            assert any(i < -0.15 for i in list(d.raw - d.pymol)), "max: " + str(max(list(d.raw - d.pymol))) + " and min: " + str(min(list(d.raw - d.pymol)))
            assert any(i < -0.15 for i in list(d.imgt - d.pymol)), "max: " + str(max(list(d.imgt - d.pymol))) + " and min: " + str(min(list(d.imgt - d.pymol)))
            assert all(abs(i) < 0.15 for i in list(d.raw - d.imgt)), "max: " + str(max(list(d.raw - d.imgt))) + " and min: " + str(min(list(d.raw - d.imgt)))
